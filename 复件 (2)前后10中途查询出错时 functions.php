...
function get_num_appear_id_interval(){
				...
				if($i + 10 < $end_n)
				{
					...
					for($j=($i-10); $j<($i+10); $j++)
					{
						if($j == $i)
						{
							...
							echo "<button><font size='5' color='blue'>$method_array[$j]</font></button><br>";
						}else{
...
//输出方法出现时前后10次的方法情况-------此组数据属于的方法编号，针对的方法（奇偶或除3余）的历史所有出现的记录数组
function print_twenty_infors($method_id, $method_array)
{
	$end_n = count($method_array);
	//echo '-=\-\=-\='.$end_n;
				...
				if($i + 10 < $end_n)
				{
					...
					for($j=($i-10); $j<($i+10); $j++)
					{
						...
						// {
							...
							// // echo "<button><font size='5' color='blue'>$method_array[$j]</font></button><br>";
							// echo "<button><font size='5' color='blue'>$method_array[$j][0], $odd_even_string, $method_array[$j][8], $method_array[$j][9] </font></button><br>";
						// }else{
							// // echo "$method_array[$j]<br>";
							// echo "$method_array[$j][0], $odd_even_string, $method_array[$j][8], $method_array[$j][9] <br>";
...
{
	$sql = "SELECT one, two, three, four, five, six, odd_even_proportion_id FROM method_odd_even where id = $odd_even_id";// by goods_id desc limit ". ($page-1)*$page_size .", $page_size";    
	$result = mysql_query($sql);
