...
{
		//$sql = "select * from data_core  where odd_even_id = $i";
		$sql = "select * from data_core  where odd_even_id = 4";
		$sql = "select id, one, two from data_core  where odd_even_id = 4";
		$sql = "select id from data_core  where odd_even_id = 4";
		echo '这里查询的列数不同，结果不同，不知道是为什么。<br>';
		...
		echo count(mysql_fetch_row($count_result)).'结果的一条记录---------90=-0-=-0=-0<br>';
		echo count(mysql_fetch_array($count_result)).'结果作为一个数组---------90=-0-=-0=-0<br>';
		echo count(mysql_fetch_assoc($count_result)).'结果的一条结果记录做成一个关联数组返回---------90=-0-=-0=-0<br>';
		
		/* 
		//echo count(mysql_fetch_field($count_result)).'列数---------90=-0-=-0=-0<br>';
		echo mysql_fetch_row($count_result).'结果的一条记录---------90=-0-=-0=-0<br>';
	...
	for($i=1; $i<65; $i++)
	{
		...
		$count_odd_even[$i] =  count($row);
		echo '总条数：'.$count_odd_even[$i].'<br>';
		
		$row = $row = mysql_fetch_row($count_result);
		$count_odd_even[$i] =  count($row);
		echo '总条数：'.$count_odd_even[$i].'<br>';
		...
		echo '总条数：'.$k.'<br>';
		echo "<br><br><br>";
		//$sql = "INSERT INTO counts_odd_even (id, method_id, count) VALUES ('',  ".$i.', '.$count_result.")";
	...
	for($i=1; $i<65; $i++)
	{
		for($j = ($i+1); $j<65; $j++)
		{
			if($count_odd_even[$i] < $count_odd_even[$j])
			{
...
function odd_even_dispose($array, $i_start, $n_end)
{
	...
	for($i=$i_start; $i<$n_end; $i++)             //循环计数器可以用变量？？？？？？？
	{
		...
		*/
		...
		//$sql = "INSERT INTO method_odd_even (odd_even_id, odd_even_proportion_id, two, three, four, five, six) VALUES ('', ".$i.', '.$j.', '.$k.', '.$l.', '.$m.', '.$n.")";
		//$sql = "INSERT INTO method_odd_even (id, one, two, three, four, five, six) VALUES ('', ".$i.', '.$j.', '.$k.', '.$l.', '.$m.', '.$n.")";
		//"INSERT INTO mytable (title, name, date) VALUES ('My title', 'My name', 'My date') ";
		//$sql = "select * from goods_book order by goods_id desc limit ". ($page-1)*$page_size .", $page_size";    
		$result = mysql_query($sql);
